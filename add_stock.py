...
class Database:
    def __init__(self,master,*args,**kwargs):
         self.master=master
         self.heading=Label(master,text="STOCK ADDING",font=('arial 40 bold'),fg='steelblue')
         ...
         self.cp_l.place(x=20, y=220)


        #enteries for window

         self.name_e=Entry(master,width=27,font=('arial 18 bold'))
         self.name_e.place(x=350,y=120)

         self.stock_e = Entry(master, width=27, font=('arial 18 bold'))
         self.stock_e.place(x=350, y=170)

         self.cp_e = Entry(master, width=27, font=('arial 18 bold'))
         self.cp_e.place(x=350, y=220)

         #button to add to the database
         self.btn_add=Button(master,text='Add to Database',width=25,height=2,bg='steelblue',fg='white',command=self.get_items)
         self.btn_add.place(x=520,y=270)

         self.btn_clear=Button(master,text="Clear All Fields",width=18,height=2,bg='red',fg='white',command=self.clear_all)
         self.btn_clear.place(x=350,y=270)
    ...
    # dynamic entries
       if self.name == '' or self.stock == '' or self.cp == '':
        ...
        mycursor.execute("INSERT INTO inventory (id,name, stock, price) VALUES(%s,%s,%s,%s)",[self.id,self.name,self.stock,self.cp])
        conn.commit()
        # textbox insert
