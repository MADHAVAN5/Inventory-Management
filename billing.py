...
class Application():
    def __init__(self,master,*args,**kwargs):
        ...
        self.left=Frame(master,width=700,height=768,bg='#FFD400',cursor='')
        self.left.pack(side=LEFT)

        self.right = Frame(master, width=666, height=768, bg='black',cursor='')
        self.right.pack(side=RIGHT)


        #components
        self.heading=Label(self.left,text="SUPER MARKET:",font=('arial 40 bold'),fg='black',bg='#FFD400')
        ...
        #enter stuff
        self.enterid=Label(self.left,text="Enter Product Name:",font=('arial 18 bold'),fg='white',bg='black')
        self.enterid.place(x=0,y=80)


        self.enteride=Entry(self.left,width=25,font=('arial 18 bold'),bg='white',cursor='heart')
        self.enteride.place(x=250,y=80)
        self.enteride.focus()

        #search button
        self.search=PhotoImage(file='search.png')
        self.search_btn=Button(self.left,image=self.search,bg='#FFD400',borderwidth=0,command=self.ajax)
    ...
    def ajax(self,*args,**kwargs):
        self.conn = mysql.connector.connect(host='localhost',
                                       ...
                                       user='root',
                                       password='')
        self.get_name=self.enteride.get()
        #get the product info with that id and fill i the labels above
        self.mycursor = self.conn.cursor()
        self.mycursor.execute("SELECT * FROM inventory WHERE name= %s",[self.get_name])
        ...
        if self.pc:
          ...
          self.productname.configure(text="Product's Name: " +str(self.get_name.upper()),fg='black',bg='#FFD400')
          self.pprice.configure(text="Price: RS. "+str(self.get_price),fg='black',bg='#FFD400')
        ...
        #add to cart button
          self.add_to_cart_img= PhotoImage(file='add_to_cart.png')
          self.add_to_cart_btn = Button(self.left,image=self.add_to_cart_img,borderwidth=0,bg='#FFD400',command=self.add_to_cart)
        ...
        #genrate bill and change
          ...
          self.change= PhotoImage(file='change.png')
          self.change_btn= Button(self.left,image=self.change,bg='#FFD400',borderwidth=0,command=self.change_func)
    ...
    def add_to_cart(self,*args,**kwargs):
        ...
        if  self .quantity_value >int(self.get_stock):
            ...
            for self.p in products_list:
                ...
                #total confugure
                self.total_l.configure(text="Total : Rs. "+str(sum(product_price)),bg='black',fg='white')
                #delete
...
root.title("Inventory Management System by Maddy")
root.mainloop()
